...
from ..models.departments import DepartmentStats
...
async def get_department_leaderboard():
    ...
    # Get departments from the database, not hardcoded list
    ...
    for dept in departments:
        ...
        # Get stats for this department using the actual DB ObjectId string
        total = await db.complaints.count_documents({"department.id": dept_id})
        ...
        # Prioritize resolution rate and number of resolved complaints
    ...
    # Sort by multiple criteria: score, resolution_rate, resolved count
    rankings.sort(key=lambda x: (x.score, x.resolution_rate, x.resolved), reverse=True)
