...
async def register_admin(
    ...
    # Validate department against database
    try:
        department = await db.departments.find_one({"_id": ObjectId(data.department_id)})
    except:
        department = None
    ...
    return {
        ...
        "department_id": data.department_id,
        "department_name": department.get("full_name", department.get("name", "Unknown"))
    }


@router.get("/departments")
async def get_all_departments(admin: dict = Depends(get_admin_user)):
    """List all available departments from database."""
    db = get_db()
    departments = await db.departments.find({}).to_list(length=100)
    return [
        {
            "id": str(d["_id"]),
            "name": d.get("full_name", d.get("name", "Unknown")),
            "short_name": d.get("name", d.get("short_name", "?"))
        }
...
async def get_department_complaints(
    ...
    is_super_admin = admin_role in [UserRole.SUPER_ADMIN.value, "super_admin"]
    
    if not is_super_admin and admin.get("department_id"):
...
async def get_dashboard_stats(
    ...
    if not is_super_admin and admin.get("department_id"):
        dept_doc = await db.departments.find_one({"_id": ObjectId(admin["department_id"])})
        if dept_doc:
            department_name = dept_doc.get("full_name", dept_doc.get("name", "Unknown"))
...
@router.get("/dashboard/departments-overview")
async def get_departments_overview(admin: dict = Depends(get_super_admin)):
    """
    Get overview of all departments with complaint statistics.
    ...
    # Get departments from DB instead of hardcoded list
    departments = await db.departments.find({}).to_list(length=100)
    
    for dept in departments:
        dept_id = str(dept["_id"])
        dept_name = dept.get("full_name", dept.get("name", "Unknown"))
        dept_short = dept.get("name", dept.get("short_name", "?"))
        
        dept_query = {"department.id": dept_id}
        
        total = await db.complaints.count_documents(dept_query)
        pending = await db.complaints.count_documents({**dept_query, "status": "pending"})
        in_progress = await db.complaints.count_documents({**dept_query, "status": "in_progress"})
        assigned = await db.complaints.count_documents({**dept_query, "status": "assigned"})
        resolved = await db.complaints.count_documents({**dept_query, "status": "resolved"})
        rejected = await db.complaints.count_documents({**dept_query, "status": "rejected"})
    ...
    departments_stats.sort(key=lambda x: x["total_complaints"], reverse=True)
    
    return {
        "departments": departments_stats,
        "total_complaints": sum(d["total_complaints"] for d in departments_stats),
        "total_resolved": sum(d["resolved"] for d in departments_stats),
        "total_pending": sum(d["pending"] for d in departments_stats)
    }
